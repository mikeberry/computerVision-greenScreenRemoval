        ...
        for y in range(0, trimap.shape[0]):
            for x in range(0, trimap.shape[1]):
                if trimap[y, x] == 255:
                    ...
                    localForegroundMask = localForegroundMask.reshape((1,localForegroundMask.shape[0],localForegroundMask.shape[1]))
                    meanForeground = np.mean(frame[starty:endy,startx:endx][tuple(localForegroundMask==1)].reshape(-1,3),axis=0).astype(np.uint8)
